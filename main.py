...
def create_opml():
    ...
    with open(os.path.join(os.getcwd(),"rss-template-v2.txt"),'r',encoding='utf-8') as load_f:
        subscription_list_template = load_f.read();
        GMT_FORMAT = '%a, %d %b %Y %H:%M:%S GMT'
        date_created = datetime.utcnow().strftime(GMT_FORMAT);
        date_modified = datetime.utcnow().strftime(GMT_FORMAT);
        subscription_list = subscription_list_template.format(result=result, date_created=date_created, date_modified=date_modified);
        # print(subscription_list);

    # 将内容写入
    with open(os.path.join(os.getcwd(),"subscription_list_v2.opml"),'w',encoding='utf-8') as load_f:
        load_f.write(subscription_list)

    subscription_list_v1 = ""
    with open(os.path.join(os.getcwd(),"rss-template-v1.txt"),'r',encoding='utf-8') as load_f:
        subscription_list_template = load_f.read();
        subscription_list_v1 = subscription_list_template.format(result=result_v1);

    # 将内容写入
    with open(os.path.join(os.getcwd(),"subscription_list_v1.opml"),'w',encoding='utf-8') as load_f:
        load_f.write(subscription_list_v1)
...
def main():
    ...
    try:
        send_mail(email_list, "RSS聚合", reResult)
    except Exception as e:
        print("==邮件设信息置错误===》》", e)
